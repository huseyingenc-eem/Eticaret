            ...
            {sidebar.map((route) => (
                <li className="relative" key={route.name}>
                    <NavLink
                        to={route.path}
                        className={({ isActive }) =>
                            `px-6 py-4 inline-flex items-center w-full text-sm font-semibold transition-colors duration-150 ${
                                isActive ? 'text-green-500 dark:text-gray-100' : 'hover:text-green-700 dark:hover:text-gray-200'
                            }`
                        }
                    >
                        {({ isActive }) =>
